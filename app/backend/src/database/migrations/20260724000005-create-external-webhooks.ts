...
module.exports = {
  up: async (queryInterface: QueryInterface) => {
    await queryInterface.createTable(subscriptions, {
      ...
      companyId: { type: DataTypes.INTEGER, allowNull: false, references: { model: "Companies", key: "id" }, onDelete: "CASCADE", onUpdate: "CASCADE" },
      name: { type: DataTypes.STRING, allowNull: false },
      url: { type: DataTypes.TEXT, allowNull: false },
      enabled: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: true },
      events: { type: DataTypes.JSONB, allowNull: false },
      connectionIds: { type: DataTypes.JSONB, allowNull: false, defaultValue: [] },
      messageKinds: { type: DataTypes.JSONB, allowNull: false, defaultValue: [] },
      includeApiOrigin: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: false },
    ...
    await queryInterface.addIndex(subscriptions, ["companyId", "enabled", "pausedAt"], { name: "webhook_subscriptions_dispatch" });

    await queryInterface.createTable(deliveries, {
      id: { type: DataTypes.UUID, primaryKey: true, allowNull: false },
      subscriptionId: { type: DataTypes.UUID, allowNull: false, references: { model: subscriptions, key: "id" }, onDelete: "CASCADE", onUpdate: "CASCADE" },
      companyId: { type: DataTypes.INTEGER, allowNull: false, references: { model: "Companies", key: "id" }, onDelete: "CASCADE", onUpdate: "CASCADE" },
    ...
    await queryInterface.addIndex(deliveries, ["subscriptionId", "eventId"], { name: "webhook_deliveries_subscription_event_unique", unique: true });
    await queryInterface.addIndex(deliveries, ["status", "availableAt", "leaseExpiresAt"], { name: "webhook_deliveries_dispatch" });
  },
