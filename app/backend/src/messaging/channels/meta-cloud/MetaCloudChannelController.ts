...
const publicBackendUrl = (): string => {
  ...
  if (!backendUrl) {
    throw new AppError("BACKEND_URL não configurada para o callback Meta", 500);
  }
  return backendUrl.replace(/\/$/, "");
};

export const createMetaCloudChannelHandler =
  () =>
...
export const rotateMetaCloudChannelHandler = async (
  ...
  res.json(
    await new MetaCloudChannelAdminService().rotate(
      req.user.companyId,
