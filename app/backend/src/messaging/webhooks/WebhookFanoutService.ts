...
interface WebhookFanoutDependencies {
  ...
  createDelivery(data: Record<string, unknown>, transaction: any): Promise<unknown>;
  completeEvent(id: string, transaction: any): Promise<unknown>;
}

const deliverableEvents = ["message.received", "message.sent", "message.status.updated", "ticket.created", "ticket.updated", "contact.updated"];

const defaultDependencies: WebhookFanoutDependencies = {
  transaction: callback => sequelize.transaction(callback),
  claimEvent: () => sequelize.transaction(async transaction => {
    const event = await MessagingOutboxEvent.findOne({
      where: { eventType: { [Op.in]: deliverableEvents }, status: "ready", availableAt: { [Op.lte]: new Date() } },
      order: [["createdAt", "ASC"]], transaction, lock: transaction.LOCK.UPDATE, skipLocked: true
    });
    if (!event) return null;
    await event.update({ status: "processing", leaseExpiresAt: new Date(Date.now() + 120000) }, { transaction });
    return event.toJSON() as DomainEvent;
  }),
  findSubscriptions: (companyId, transaction) => WebhookSubscription.findAll({
    where: { companyId, enabled: true, pausedAt: null },
    transaction
  }),
  createDelivery: (data, transaction) => WebhookDelivery.create(data as any, { transaction }),
  completeEvent: (id, transaction) => MessagingOutboxEvent.update(
...
const matches = (subscription: any, event: DomainEvent): boolean => {
  ...
  if (event.payload.origin === "api" && !subscription.includeApiOrigin) return false;
  if (subscription.connectionIds?.length && !subscription.connectionIds.includes(event.payload.whatsappId)) return false;
  if (subscription.messageKinds?.length && !subscription.messageKinds.includes(event.payload.kind)) return false;
  return true;
};

class WebhookFanoutService {
  constructor(private readonly dependencies: WebhookFanoutDependencies = defaultDependencies) {}

  async fanoutOne(): Promise<{ status: "idle" | "created"; deliveries: number }> {
    const event = await this.dependencies.claimEvent();
    if (!event) return { status: "idle", deliveries: 0 };
    const deliveries = await this.dependencies.transaction(async transaction => {
      const subscriptions = await this.dependencies.findSubscriptions(event.companyId, transaction);
      let created = 0;
      for (const subscription of subscriptions.filter(item => matches(item, event))) {
