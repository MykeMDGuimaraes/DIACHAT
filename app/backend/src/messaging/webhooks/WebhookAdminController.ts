...
const defaults: WebhookAdminDependencies = {
  ...
  list: companyId => WebhookSubscription.findAll({
    where: { companyId },
  ...
  remove: async (companyId, id) => {
    const subscription = await WebhookSubscription.findOne({ where: { id, companyId } });
  ...
  listDeliveries: (companyId, query) => {
    ...
    return WebhookDelivery.findAll({ where, order: [["createdAt", "DESC"]], limit: 100 });
  },
  retryDelivery: async (companyId, id) => {
    const delivery = await WebhookDelivery.findOne({ where: { id, companyId } });
    if (!delivery) throw new AppError("Entrega de webhook nÃ£o encontrada", 404);
    if (delivery.status !== "dead_letter") throw new AppError("A entrega nÃ£o estÃ¡ em dead-letter", 409);
    await delivery.update({ status: "ready", attemptCount: 0, availableAt: new Date(), leaseExpiresAt: null, lastError: null });
  }
};

export const createWebhookAdminHandlers = (dependencies: WebhookAdminDependencies = defaults) => ({
  create: async (req: Request, res: Response): Promise<Response> => {
    const result = await dependencies.create({ ...req.body, companyId: req.user.companyId });
    return res.status(201).json(result);
  },
  list: async (req: Request, res: Response): Promise<Response> =>
    res.json(await dependencies.list(req.user.companyId)),
  update: async (req: Request, res: Response): Promise<Response> =>
    res.json(await dependencies.update({ ...req.body, companyId: req.user.companyId, id: req.params.subscriptionId })),
  ...
  listDeliveries: async (req: Request, res: Response): Promise<Response> =>
    res.json(await dependencies.listDeliveries(req.user.companyId, req.query as any)),
